...
describe('AuctionFactory', () => {
  ...
  async function launchAuction() {
    const { auctionFactory, mockNFT, mockToken } = await loadFixture(
      deployContract
    );
  ...
  async function bid() {
    ...
    await auctionFactory.functions['bid(uint256,uint256)'](
      1,
      balanceOwner.toString()
    );

    const balanceAddr1 = await mockToken.balanceOf(addr1.address);
    await mockToken
      .connect(addr1)
      .approve(auctionFactory.address, balanceAddr1.toString());
    await auctionFactory
      .connect(addr1)
      .functions['bid(uint256,uint256)'](1, balanceAddr1.toString());
  ...
  it('Bid on Auction', async function () {
    var { auctionFactory, mockNFT, mockToken } = await loadFixture(
      depositERC721
    );
    const [owner] = await ethers.getSigners();
    ...
    const bidderBalance = await auctionFactory.getBidderBalance(
      1,
      owner.address
    );
